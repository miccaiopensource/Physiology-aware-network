...
def snake_collator(batch):
    # print("batch:",batch)
    # ret = {'inp': default_collate([b['inp'] for b in batch]),'mask': default_collate([b['mask'] for b in batch]),'mask_seg': default_collate([b['mask_seg'] for b in batch]),'path': default_collate([b['path'] for b in batch]),
    #        'risk_label':default_collate([b['risk_label'] for b in batch]),'thickness_set':default_collate([b['thickness_set'] for b in batch])}

    # ret = {'inp': default_collate([b['inp'] for b in batch]), 'mask': default_collate([b['mask'] for b in batch]),
    #        'mask_seg': default_collate([b['mask_seg'] for b in batch]),
    #        'path': default_collate([b['path'] for b in batch]),'mask_dis': default_collate([b['mask_dis'] for b in batch])}

    # ret = {'inp': default_collate([b['inp'] for b in batch]), 'mask': default_collate([b['mask'] for b in batch]),
    #        'mask_seg': default_collate([b['mask_seg'] for b in batch]),
    #        'path': default_collate([b['path'] for b in batch]),
    #        'mask_dis': default_collate([b['mask_dis'] for b in batch]),
    #        'mask_seg_plaque': default_collate([b['mask_seg_plaque'] for b in batch])}

    ret = {'inp': default_collate([b['inp'] for b in batch]), 'mask': default_collate([b['mask'] for b in batch]),
           'mask_seg': default_collate([b['mask_seg'] for b in batch]),
           'path': default_collate([b['path'] for b in batch]),
           'mask_dis': default_collate([b['mask_dis'] for b in batch]),
           'mask_seg_wall': default_collate([b['mask_seg_wall'] for b in batch]),
           'mask_seg_plq_only': default_collate([b['mask_seg_plq_only'] for b in batch]),
           'label_health': default_collate([b['label_health'] for b in batch])}
    ...
    ret.update(init)
    # print("collect_i_it_4pys_len:", len(i_it_4pys))
    ...
    # print("collect_c_it_pys_len:",len(c_it_pys))
    # print("collect_i_gt_py_shape:", i_gt_pys.shape)
    # print("collect_i_gt_py_len:", len(i_gt_pys))
    # print("collect_i_gt_py_len:", len(i_gt_pys))
...
def ext_snake_collator(batch):
    ret = {'inp': default_collate([b['inp'] for b in batch]),'poly': default_collate([b['poly'] for b in batch])}
